...
| `-9` | `EBADF` | 未知のfile descriptor、標準streamへの`close`/`lseek`、未割り当てfdへの`read`/`write`/`pread`/`pwrite`/`lseek`/`close`、writable fdへの`read`/`pread`、read-only fdへの`write`/`pwrite`、`unlink`で失効したfdへの操作 |

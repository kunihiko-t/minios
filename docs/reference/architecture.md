...
- `memory/frame.rs`：`PhysFrame`、`FrameError`、`FrameStats`、const genericsを使う`FrameAllocator`、供給元を抽象化する`FrameSource` traitを提供します。
...
- `memory/heap.rs`：managed RAM末尾の1 MiB初期領域を16バイト粒度で分割するfirst-fit free-listヒープを提供します。
  ...
  空きが尽きると`extend_down`がframe allocatorの最上位pageを取り込んで領域を下方へ伸ばすため、ヒープとprocess frameは同一poolを動的に分け合います。
...
- `user/syscall.rs`：`a7`の番号と`a0`〜`a3`のargumentsから`write`、`exit`、`read`、`read_file`、`open`、`close`、`create`、`unlink`、`lseek`、`pread`、`pwrite`、`rename`、`mkdir`、`rmdir`、`getpid`、`spawn`、`waitpid`、`stat`、`fstat`、`readdir`をdispatchします。
...
- `cli.rs`：`setup`、`build`、`run`、`bundle`、`test`、`check`と、user-entry、user-trap、user-syscall、user-exit、payload、payload-args、payload-stdin、file、file-fd、file-write、file-unlink、file-seek、file-rename、file-mkdir、file-spawn、file-waitpid、file-stat、file-readdir、schedを含む引数構文を定義します。
...
- `storage/virtio_blk.rs`：virtio-mmio v2のblock driverであり、feature negotiation、単一queue、3-desc chainによるsector読み取りと書き込み、used ringのpollを行います。

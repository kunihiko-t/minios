...
NEORV32では同じ`console`の下で[`Neorv32Uart`](../../kernel/src/drivers/neorv32_uart.rs)を使います。
UART0の制御レジスターは`0xfff5_0000`、データレジスターは`0xfff5_0004`にあり、96 MHzの基準クロックから19,200 baudの設定値を計算します。
送信FIFOの満杯状態と受信データの有無を32 bitの制御レジスターで調べるため、QEMUの16550互換UARTとはオフセットとビットの意味が異なります。
...
- NEORV32で文字化けする：FPGA構成の基準クロックと端末のボーレートが、実装の96 MHzおよび19,200 baudと一致するか確認します。

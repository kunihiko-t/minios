...
cargo xtask test [all|boot|trap|timer|memory|vm|elf|user-entry|user-trap|user-syscall|user-exit|fdt|heap|virtio|payload|payload-args|payload-stdin|file|file-fd|file-write|file-unlink|file-seek|file-rename|file-mkdir|file-spawn|file-waitpid|file-stat|sched|sched-io|sched-io-partial|shell]
cargo xtask check
```
...
29. QEMU schedテスト
30. QEMU sched-ioテスト
31. QEMU sched-io-partialテスト
32. QEMUシェルテスト

速いホストテストを先に実行してから、起動、トラップ、タイマー、メモリー、VM、ELF、U-mode、FDT、ヒープ、VirtIO block、payload、payload-args、payload-stdin、file読み取り、file descriptor、file書き込み、file削除、位置指定I/O、file改名、directory作成と削除、process起動、スケジューラー、stdin待ちprocessを含むスケジューラー、分割frame受信、対話シェルという依存関係の順にゲストの29経路を確認します。
...
`user-exit`、`payload`、`payload-args`、`payload-stdin`、`file`、`file-fd`、`file-write`、`file-unlink`、`file-seek`、`file-rename`、`file-mkdir`、`file-spawn`、`file-waitpid`、`file-stat`、`sched`、`sched-io`は**control frameモード**です。
...
directoryのrenameについては、`mkdir`したdirを改名して中のfileが新dir名で読めること、dir→非空dirが`ENOTEMPTY`、dir→空dirが置き換えになること、片付けに`rmdir`が使えることを確かめます。
cross-directory moveについては、fileをdirへ移して旧名が`ENOENT`・新名で読める往復、開いているwritable fdを持つfileを移してfd経由の追記が新しいdir entryへwrite-backされること、別dir内の既存fileへの移動が置き換えとなりtargetのfdが`EBADF`で失効すること、fileをdirごと別のdirの中へ移して中身を新pathで読みrootへ戻すこと（`..`の更新）、dirを自身や子孫の中へ移す指定が`EINVAL`、不在の親dirへの移動が`ENOENT`であることを確かめてから、標準出力へ出して終了コード42を返すことを要求します。
...
`file-spawn`ではfile_spawn guestが`getpid`で自分のpid 0を受け取り、`spawn`で`DOCS/CHILD.ELF`を起動してchildのpid 1を受け取ることを確かめます。
disk image内の`CHILD.ELF`は最小限のELF64で、起動したchild processが`spawn-child`を標準出力へ書き、`getpid`で得たpidに41を足した42で終了します。
不在pathの`ENOENT`、directoryの`EISDIR`、非ELF fileの`EINVAL`、空pathの`EINVAL`も確かめます。
childは親の残りの実行とどちらが先に進んでもよいため、検証部はframe順ではなく、二つのstdout markerと二つのExit(42) frameが揃うことを要求します。
`file-waitpid`ではfile_waitpid guestが同じ`DOCS/CHILD.ELF`を`spawn`し、`waitpid`でchildの終了code 42を回収することを確かめます。
reap済みpid・自分自身・存在しないpidへの`waitpid`が`ECHILD`を返すことも確かめます。
...
`mv`はfileとdirectoryの両方を改名し、file改名後の`ls`が新名を列挙し、dir改名後は新しいdir名のまま`cat`で中身へ到達できること、fileをdirectoryの中へ移して新pathで読みrootへ戻せることを確認します。
...
`cargo xtask check`は、次の44段階をこの順に実行し、最初の失敗で停止します。
...
42. QEMU sched test
43. QEMU sched-io test
44. QEMU sched-io-partial test
45. QEMU shell test
```

各見出しは`[現在/総数]`、各段階の結果は経過時間を表示します。
全段階に成功すると`summary: PASSED all 45 phases`を表示します。
...
```console
$ cargo xtask check
[1/45] cargo fmt --all -- --check
phase 1/45 passed (elapsed: ...s)
...
[45/45] QEMU shell test
phase 45/45 passed (elapsed: ...s)
summary: PASSED all 45 phases (elapsed: ...s)
```

...
初めて読むときは第1章から第14章まで順に進み、実装中は各章の「実行と確認」と「よくある失敗」を参照してください。
...
11. [テストハーネスの仕組み](11-test-harness.md)：ホスト、QEMU、CIを同じ19段階で検証する。
12. [次に作るもの](12-next-steps.md)：完了した順序変更と次のU-mode段階を依存関係で説明する。
...
第1章の前と第14章の次は、この索引です。
`01-...md`から`14-...md`までの各章は、「学習目標」「背景」「実装」「実行と確認」「よくある失敗」「演習」「次の章」の七つの節を持ちます。

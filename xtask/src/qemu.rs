...
pub fn run_test(kind: TestKind, deadline: Duration) -> Result<String, QemuError> {
    ...
    if kind == TestKind::FileReaddir {
        let kernel = cargo::build_kernel(false).map_err(QemuError::Build)?;
        let bundle = PayloadBundle::create_file_readdir()?;
        let disk = crate::disk::DiskImage::create().map_err(|error| QemuError::Bundle {
            stage: "disk image",
            error,
        })?;
        ...
        bundle.remove();
        disk.remove();
        return verify_file_readdir_result(
            &command_line,
            completed.status.code(),
            &completed.output,
        );
...
/// file-readdir検査で期待されるcontrol frame列。guestがreaddirの
/// index列挙・kind・EOF・errno・EFAULTの経路を通してから、検証済みの
/// 旨をstdoutへ出力する。
const FILE_READDIR_EXPECTED_FRAMES: [&[u8]; 5] = [
    PAYLOAD_READY_FRAME,
    FILE_READDIR_SPAWNED_FRAME,
    FILE_READDIR_STDOUT_FRAME,
    PAYLOAD_EXIT_FRAME,
    PAYLOAD_DIAGNOSTIC_FRAME,
];
...
/// file-readdir検証: guestがreaddirの契約を全て確認し、`readdir
/// verified`とExit(42)を出す。単一processなのでexact照合できる。
fn verify_file_readdir_result(
    command: &str,
    status: Option<i32>,
    output: &str,
) -> Result<String, QemuError> {
...
impl PayloadBundle {
    ...
    fn create_file_readdir() -> Result<Self, QemuError> {
        let elf = built_bin_elf_bytes(crate::guest::GUEST_FILE_READDIR)?;
...
/// file-readdir検査用bundle: file_readdir guestのELFと引数なしmanifestを
/// 組み立てる。guestはreaddirのindex列挙・kind・EOF・errno・EFAULT

...
impl Phase {
    ...
    fn command(self) -> String {
        ...
        match self {
            ...
            Self::Qemu(qemu::TestKind::File) => "QEMU file test".to_owned(),
            Self::Qemu(qemu::TestKind::FileFd) => "QEMU file-fd test".to_owned(),
            Self::Qemu(qemu::TestKind::FileWrite) => "QEMU file-write test".to_owned(),
            ...
            Self::Qemu(qemu::TestKind::FileSpawn) => "QEMU file-spawn test".to_owned(),
...
fn test_phases() -> Vec<Phase> {
    vec![
        ...
        Phase::Qemu(qemu::TestKind::File),
        Phase::Qemu(qemu::TestKind::FileFd),
        Phase::Qemu(qemu::TestKind::FileWrite),
        ...
        Phase::Qemu(qemu::TestKind::FileSpawn),
...
fn phase_plan_for(command: &Command) -> Option<Vec<Phase>> {
    match command {
        ...
        Command::Test(TestFilter::PayloadStdin) => {
            Some(vec![Phase::Qemu(qemu::TestKind::PayloadStdin)])
        }
        Command::Test(TestFilter::File) => Some(vec![Phase::Qemu(qemu::TestKind::File)]),
        Command::Test(TestFilter::FileFd) => Some(vec![Phase::Qemu(qemu::TestKind::FileFd)]),
        Command::Test(TestFilter::FileWrite) => Some(vec![Phase::Qemu(qemu::TestKind::FileWrite)]),
        Command::Test(TestFilter::FileUnlink) => {
            Some(vec![Phase::Qemu(qemu::TestKind::FileUnlink)])
        }
        Command::Test(TestFilter::FileSeek) => Some(vec![Phase::Qemu(qemu::TestKind::FileSeek)]),
        ...
        Command::Test(TestFilter::FileSpawn) => Some(vec![Phase::Qemu(qemu::TestKind::FileSpawn)]),
        Command::Test(TestFilter::FileWaitpid) => {
            Some(vec![Phase::Qemu(qemu::TestKind::FileWaitpid)])
        }
        Command::Test(TestFilter::FileStat) => Some(vec![Phase::Qemu(qemu::TestKind::FileStat)]),
...
mod tests {
    ...
    fn check_plan_runs_every_user_runtime_phase_after_host_tests() {
        ...
        assert_eq!(&plan[host_tests_end + 7..host_tests_end + 30], expected);
    ...
    fn public_vm_and_elf_filters_connect_to_named_qemu_phases() {
        ...
        assert_eq!(
            ...
            Some(vec![Phase::Qemu(qemu::TestKind::Payload)])
        );
        assert_eq!(
            phase_plan_for(&Command::Test(TestFilter::PayloadArgs)),
            Some(vec![Phase::Qemu(qemu::TestKind::PayloadArgs)])
        );
        assert_eq!(
            phase_plan_for(&Command::Test(TestFilter::PayloadStdin)),
            Some(vec![Phase::Qemu(qemu::TestKind::PayloadStdin)])
        );
        assert_eq!(
            phase_plan_for(&Command::Test(TestFilter::Sched)),
            Some(vec![Phase::Qemu(qemu::TestKind::Sched)])
        );
        assert_eq!(
        ...
        assert_eq!(
            Phase::Qemu(qemu::TestKind::PayloadArgs).command(),
            "QEMU payload-args test"
        );
    }
